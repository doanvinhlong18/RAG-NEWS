...
class HybridRetriever:
    ...
    def _get_faiss(self):
        if self._faiss is None:
            ...
            if hasattr(self._faiss, "hnsw"):
                ...
                logger.info(f"FAISS HNSW efSearch={self.hnsw_ef_search}")
    ...
    def _get_cid_to_meta(self) -> dict:
        if self._cid_to_meta is None:
            metadata = self._get_metadata()
            self._cid_to_meta = {c["chunk_id"]: c for c in metadata.values()}
        return self._cid_to_meta
    ...
    def retrieve(
        ...
        t_hybrid = time.time()
        with ThreadPoolExecutor(max_workers=min(len(queries), 4) or 1) as ex:
            def run_bm25():
                t0 = time.time()
                res = [self._bm25_search(q) for q in queries]
                dur = time.time() - t0
                return res, dur

            bm25_future = ex.submit(run_bm25)

            t_dense = time.time()
            dense_batch = self._dense_search_batch(queries, batch_size=self.dense_query_batch_size)
            dense_time = time.time() - t_dense

            bm25_batch, bm25_time = bm25_future.result()

        self.last_bm25_time = bm25_time
        self.last_dense_time = dense_time

        t_fuse = time.time()
        per_query_fused = [
            rrf_fusion([bm25_batch[i], dense_batch[i]], k=self.rrf_k, top_k=self.top_k)
            for i in range(len(queries))
        ]
        fused = rrf_fusion(per_query_fused, k=self.rrf_k, top_k=self.top_k)
        ...
        t_join = time.time()
        cid_to_meta    = self._get_cid_to_meta()
        ...
        self.last_join_time = time.time() - t_join

        logger.info(
            f"[Retrieval:Hybrid] total={time.time()-t_start:.3f}s"
            f"  parallel_hybrid={time.time()-t_hybrid:.3f}s"
            f"  dense={dense_time:.3f}s"
            f"  bm25_time={bm25_time:.3f}s"
            f"  fuse={fuse_time:.3f}s"
            f"  join={self.last_join_time:.3f}s"
            f"  variants={len(queries)}"
...
class RetrievalPipeline:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.hybrid = HybridRetriever(
            ...
            hnsw_ef_search = self.cfg.get("indexing", {}).get("hnsw_ef_search", 128),
        )
    ...
    def retrieve(self, query: str, verbose: bool = False) -> List[Dict]:
        ...
        expand_time = time.time() - t_stage
        self.last_expand_time = expand_time
        ...
        hybrid_time = time.time() - t_stage
        self.last_hybrid_time = hybrid_time
        self.last_bm25_time = getattr(self.hybrid, "last_bm25_time", 0.0)
        self.last_dense_time = getattr(self.hybrid, "last_dense_time", 0.0)
        self.last_join_time = getattr(self.hybrid, "last_join_time", 0.0)
        ...
        bi_time = time.time() - t_stage
        self.last_bi_time = bi_time
        if verbose:
        ...
        ce_time = time.time() - t_stage
        self.last_ce_time = ce_time
        self.last_total_time = time.time() - t_total
        if verbose:
            logger.info(f"After cross-encoder rerank: {len(final)} results")

        logger.info(
            f"[Retrieval Stages Timing]\n"
            f"  Query Expansion  : {self.last_expand_time:.3f}s\n"
            f"  BM25 Retrieval   : {self.last_bm25_time:.3f}s\n"
            f"  FAISS Retrieval  : {self.last_dense_time:.3f}s\n"
            f"  Metadata Join    : {self.last_join_time:.3f}s\n"
            f"  Bi-Encoder Rerank: {self.last_bi_time:.3f}s\n"
            f"  Cross-Encoder Rer: {self.last_ce_time:.3f}s\n"
            f"  Total Retrieval  : {self.last_total_time:.3f}s"
        )
        return final
